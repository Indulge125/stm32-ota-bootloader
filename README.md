...
- **A 区**（`0x08005000`）：应用程序。A 区工程需设置
  `IROM1 起始 0x08005000`，且 `VECT_TAB_OFFSET = 0x5000`（向量表重定位）
...
| `1.1-(A区)串口测试程序` | A 区应用程序，已按 `0x08005000` 链接 |
...
- **命令解析严格匹配长度**：串口工具带了换行符会导致命令被静默忽略，
  建议改成按行解析
- **A 区工程 IROM Size 为 `0x10000`（64 KB）**，应为 `0xB000`（44 KB，
  从 `0x08005000` 到 Flash 末尾）。当前固件仅 13 KB 无影响，
  但 A 区代码增长超过 44 KB 时链接器不会报错，运行时会写坏数据
