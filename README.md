...
**没有任何编译选项能在编译期转换字符串编码**。于是：

| 源码编码 | 串口发出的字节 | 串口助手按 GB2312 看 | GitHub 上中文注释 |
|---|---|---|---|
| UTF-8 | UTF-8 | ❌ 乱码 | ✅ 正常 |
| GBK | GBK | ✅ 正常 | ❌ 乱码 |

**两者不可兼得。** 本项目选择：源码保持 UTF-8（GitHub 可读），
**串口输出一律用 ASCII 英文** —— ASCII 不受任何编码影响，在任何终端、任何代码页下都正常显示。
这也是嵌入式固件调试输出的常见做法。

> 因此：`U1_printf()` 里不要写中文。中文只出现在注释和 OLED 显示字符串（OLED 字库本身就是 ASCII）。
> 这条约束已用脚本 `_tools/list_serial_strings.py` 做过全量核查。
...
| 命令 `6`：W25Q64 搬运到 A 区并跳转 | ✅ **已验证**（`Length: 13056 bytes` → `Region A update complete` → 跳转成功打印 `48 0 30`）|
...
**串口工具**：SecureCRT / Xshell / MobaXterm 等支持 Xmodem-CRC 的工具，
波特率 9600，**不要勾"发送新行"**。
...
- **Xmodem 尾包处理**：`boot.c` 中 EOT 后的尾包写入按固定 4 页循环，
  `UpDataBuff` 中残留的旧包数据也会被写入。因固件长度由 `FileLen` 记录、
  搬运时只读有效范围，**当前无实际影响**；但若改为按整块读取就会踩到
- **命令解析严格匹配长度**：串口工具带了换行符会导致命令被静默忽略，
  建议改成按行解析
- **BootLoader 事件环形缓冲只有 10 个槽位**（`usart.h` 的 `Num`），
  且 `BootLoader_Enter()` 死等 5 秒期间主循环不消费事件 ——
  上位机若在开机窗口内高频发送字符会冲爆环形缓冲，导致之后的数据包被静默丢弃。
  用图形化串口工具手动操作不会触发；自动化脚本需要克制发送频率
