...
void USART1_IRQHandler(void)
{
	...
	{
		USART_GetFlagStatus(USART1, USART_FLAG_IDLE);										//清除空闲中断标志位步骤1：先读USART_SR寄存器
		USART_ReceiveData(USART1);					 										//清除空闲中断标志位步骤2：再读USART_DR寄存器
		U1CB.URxCounter += (USART1_RX_MAX + 1) - DMA_GetCurrDataCounter(DMA1_Channel5);		//+=操作，将本次的接收数量，累计到URxCounter变量，DMA总量-DMA通道的剩余量=DMA接收量
		U1CB.URxDataIn->end = &USART1_RxBuff[U1CB.URxCounter - 1];							//end指针指向接收量的结尾，IN指针指向的结构体中的e指针记录本次接收的结束位置
		U1CB.URxDataIn ++;																	//In指针后移一位
		if(U1CB.URxDataIn == U1CB.URxDataEnd)		 										//判断是否到达END
		{
			U1CB.URxDataIn = &U1CB.URxDataPtr[0];											//回卷，重新指向0号成员
		}
		if(USART1_RX_SIZE - U1CB.URxCounter >= USART1_RX_MAX)								//判断，如果剩余的空间大于等于单次接收最大量，进入if
		{
			U1CB.URxDataIn->start = &USART1_RxBuff[U1CB.URxCounter];						//IN指针指向的结构体中的s指针记录下次接收的起始位置
		}
		else																				//判断，如果剩余的空间小于单次接收最大量，进入else
		{
			U1CB.URxDataIn->start = USART1_RxBuff;											//剩余空间不用了，回卷，下次的接受位置返回缓冲区起始位置
			U1CB.URxCounter = 0;															//累计值清零
		}
